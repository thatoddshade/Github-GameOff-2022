...
def display_bar(
    screen, x, y, width, height, colour, border_colour, border_width, radius=0, value=0
):
    # draw bar
    score_rect = pygame.Rect(x, y, (value / 100) * width, height)
    pygame.draw.rect(screen, colour, score_rect, border_radius=radius)

    # draw bar border
    bar_border = pygame.Rect(x, y, width, height)
    pygame.draw.rect(screen, border_colour, bar_border, border_width, radius)


def write_continent_name(screen, font, continent_name, text_colour):
    # write a text indicating the selected continent name
    text = font.render(
        continent_name.capitalize().replace("_", " "), False, text_colour
    )
